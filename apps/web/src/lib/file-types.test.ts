...
describe("isAllowedFile", () => {
  ...
  it("rejects common non-image, non-pdf types", () => {
    expect(isAllowedFile(mockFile("data.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"))).toBe(false);
    expect(isAllowedFile(mockFile("doc.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"))).toBe(false);
...
describe("filterAllowedFiles", () => {
  it("returns the allowed files and the rejected files separately", () => {
    ...
    const xlsx = mockFile("data.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
