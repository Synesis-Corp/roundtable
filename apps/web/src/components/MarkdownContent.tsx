...
// (~600 KB+ raw) while keeping syntax highlighting for the common cases.
...
export function MarkdownContent({ content }: MarkdownContentProps) {
  const components: Components = {
    h1({ children }) {
      return (
        <h1
          className="text-[24px] font-semibold mt-3 mb-3"
    ...
    h2({ children }) {
      return (
        <h2
          className="text-[20px] font-semibold mt-4 mb-2"
          style={{ color: 'var(--text-1)', lineHeight: 1.3 }}
        >
          {children}
        </h2>
      );
    ...
    h3({ children }) {
      return (
        <h3
          className="text-[17px] font-semibold mt-3 mb-1.5"
          style={{ color: 'var(--text-1)', lineHeight: 1.35 }}
        >
          {children}
        </h3>
      );
    ...
    h4({ children }) {
      return (
        <h4 className="text-[15px] font-semibold mt-2 mb-1.5" style={{ color: 'var(--text-1)' }}>
          {children}
        </h4>
      );
    },
    p({ children }) {
      return (
        <p className="mb-2" style={{ color: 'var(--text-1)', lineHeight: 1.5 }}>
          {children}
        </p>
      );
    },
    ul({ children }) {
      return (
        <ul className="mb-3 pl-5 space-y-1 list-disc" style={{ color: 'var(--text-1)' }}>
          {children}
        </ul>
      );
    },
    ol({ children }) {
      return (
        <ol className="mb-3 pl-5 space-y-1 list-decimal" style={{ color: 'var(--text-1)' }}>
          {children}
        </ol>
      );
    },
      ...
      if (isInline) {
        return (
          <code
            ...
            style={{ backgroundColor: 'var(--bg-elevated)', color: '#9BF6C3' }}
            {...props}
          >
            {children}
          </code>
