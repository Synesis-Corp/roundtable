...
function mockFetchOnce(response: { status: number; body: unknown } | { error: string }): void {
  if ('error' in response) {
    globalThis.fetch = vi
      .fn()
      .mockRejectedValueOnce(new Error(response.error)) as unknown as typeof fetch;
    return;
  }
        ...
        results: [
          {
            ...
            content: 'Top story about Ecuador politics',
          },
          {
            title: 'El Comercio',
            url: 'https://elcomercio.com/y',
            content: 'Breaking news about Ecuador',
          },
        ],
      },
    });
