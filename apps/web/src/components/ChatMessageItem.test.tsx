...
const mockCouncil = {
  members: [
    ...
    { modelId: "claude-sonnet-4", provider: "anthropic", displayName: "Claude Sonnet 4", color: "#cf9a5e" },
  ...
  consensus: true,
  votes: [
    { modelId: "gpt-5", provider: "openai", displayName: "GPT-5", approachLabel: "Propone índices con cobertura parcial y desnormalizar los contadores de \"no leídos\". Su solución gana la votación.", vote: "for" as const, isWinner: true },
    { modelId: "claude-sonnet-4", provider: "anthropic", displayName: "Claude Sonnet 4", approachLabel: "Coincide; refuerza con particionado por fecha y un caso límite de borrado en cascada.", vote: "for" as const, isWinner: false },
    { modelId: "deepseek-v4", provider: "deepseek", displayName: "DeepSeek V4", approachLabel: "Defendió un esquema sin índices compuestos; cambió su voto en la ronda 2 tras los benchmarks.", vote: "changed" as const, isWinner: false },
    { modelId: "gemini-2.5", provider: "google", displayName: "Gemini 2.5", approachLabel: "Aporta consideraciones de coste de almacenamiento; vota a favor de la solución de GPT-5.", vote: "for" as const, isWinner: false },
  ],
  answer: "El consejo recomienda una tabla **messages** particionada por mes, con un índice parcial sobre los no leídos y un contador desnormalizado por conversación.",
  plannedRounds: 3,
  currentRound: 3,
...
describe("ChatMessageItem", () => {
  ...
  it("renders an assistant message as markdown HTML without a model chip", () => {
    const { container } = renderItem({ id: "2", role: "assistant", content: "**bold**", provider: "openai", model: "gpt-4o" });
  ...
  it("renders a multi-provider answer as a clean assistant message", () => {
    renderItem({ id: "5", role: "assistant", content: "synthesis", provider: "multi", model: "multi" });
  ...
  it("shows a collapsible reasoning block when the message has reasoning", async () => {
    renderItem({ id: "r1", role: "assistant", content: "final answer", reasoning: "step one then step two", provider: "openai", model: "gpt-4" });
  ...
  it("shows Copy and Feedback buttons on assistant messages", () => {
    renderItem({ id: "6", role: "assistant", content: "assistant reply", provider: "openai", model: "gpt-4" });
  ...
  it("toggles feedback buttons mutually exclusively", async () => {
    renderItem({ id: "11", role: "assistant", content: "reply", provider: "openai", model: "gpt-4" });
  ...
  it("renders CouncilBlock when provider is 'council'", () => {
    renderItem({ id: "12", role: "assistant", content: "council answer", provider: "council", councilInfo: mockCouncil });
  ...
  it("renders CouncilBlock when provider is 'consensus'", () => {
    renderItem({ id: "13", role: "assistant", content: "consensus answer", provider: "consensus", councilInfo: mockCouncil });
    expect(screen.getByText("Consejo · 4 modelos")).toBeInTheDocument();
  });

  it("opens and closes the council deliberation explorer", async () => {
    renderItem({ id: "14", role: "assistant", content: "council", provider: "council", councilInfo: mockCouncil });
  ...
  it("shows a real loading placeholder instead of mock council data", () => {
    renderItem({ id: "15", role: "assistant", content: "", provider: "council" }, { streaming: true, isLast: true });
    ...
    renderItem({
      ...
      toolCalls: [
        ...
        },
        {
          ...
          result: {
            query: "ecuador hoy",
            results: [{ title: "El Universo — portada", url: "https://www.eluniverso.com/x", snippet: "" }],
          },
        },
      ],
    });
