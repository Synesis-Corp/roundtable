...
export interface UseSettingsReturn {
  ...
  handleConnect: (providerId: string, apiKey: string, options?: Record<string, unknown>) => Promise<void>;
  requestDisconnect: (providerId: string) => void;
  handleDisconnectConfirmed: () => Promise<void>;
  setPendingDisconnect: React.Dispatch<React.SetStateAction<{ providerId: string; name: string } | null>>;
  handleCodexStart: () => Promise<void>;
  setSaveMessages: React.Dispatch<React.SetStateAction<Record<string, { text: string; type: "success" | "error" }>>>;
  setCodexNotice: React.Dispatch<React.SetStateAction<{ text: string; type: "success" | "error" } | null>>;
}
...
export function useSettings(): UseSettingsReturn {
  ...
  const [saveMessages, setSaveMessages] = useState<Record<string, { text: string; type: "success" | "error" }>>({});
  const [testing, setTesting] = useState<Record<string, boolean>>({});
  const [codexConnecting, setCodexConnecting] = useState(false);
  const [codexNotice, setCodexNotice] = useState<{ text: string; type: "success" | "error" } | null>(null);
  const [pendingDisconnect, setPendingDisconnect] = useState<{ providerId: string; name: string } | null>(null);
  ...
  const testConnection = useCallback(
    async (providerId: string, apiKey: string) => {
      ...
      try {
        const data = await apiPost<{ success: boolean; error?: string }>("/providers/test-connection", { providerId, apiKey: apiKey || undefined });
        setSaveMessages((prev) => ({
          ...prev,
          [providerId]: { text: data.success ? "Connection successful!" : (data.error || "Connection failed"), type: data.success ? "success" : "error" },
        }));
      } catch (err) {
        setSaveMessages((prev) => ({
          ...prev,
          [providerId]: { text: err instanceof Error ? err.message : "Network error during test", type: "error" },
        }));
      } finally {
  ...
  const handleConnect = useCallback(
    async (providerId: string, apiKey: string, options?: Record<string, unknown>) => {
      ...
      try {
        await apiPost("/providers", { providerId, apiKey, options: options && Object.keys(options).length > 0 ? options : undefined });
        clearIsNewFlag();
        setSaveMessages((prev) => ({ ...prev, [providerId]: { text: "Connected successfully!", type: "success" } }));
        fetchUserProviders();
        ...
        setSaveMessages((prev) => ({
          ...prev,
          [providerId]: { text: err instanceof Error ? err.message : "Failed to connect", type: "error" },
        }));
      } finally {
  ...
  const handleCodexStart = useCallback(async () => {
    ...
    try {
      const data = await apiPost<{ authorizationUrl?: string }>("/providers/openai/codex/start", {});
      ...
      setCodexNotice({ text: err instanceof Error ? err.message : "Network error starting Codex login", type: "error" });
    } finally {
