...
const mockModels: ModelInfo[] = [
  { id: "gpt-4o", name: "GPT-4o", provider: "openai", description: "Flagship model", contextWindow: 128000, capabilities: ["text", "vision"] },
  { id: "gpt-4o-mini", name: "GPT-4o Mini", provider: "openai", description: "Fast and affordable", contextWindow: 128000, capabilities: ["text"] },
  { id: "claude-3-opus", name: "Claude 3 Opus", provider: "anthropic", description: "Most capable", contextWindow: 200000, capabilities: ["text"] },
  { id: "claude-3-haiku", name: "Claude 3 Haiku", provider: "anthropic", description: "Fast", contextWindow: 200000, capabilities: ["text"] },
];
...
function renderModal(props: Partial<Parameters<typeof CouncilMembersModal>[0]> = {}) {
  ...
  return render(
    <CouncilMembersModal {...defaultProps} {...props} />
  );
}
...
describe("CouncilMembersModal", () => {
  ...
  it("updates counter when selecting models", () => {
    renderModal();
    const checkboxes = screen.getAllByRole("checkbox").filter((cb) => cb.getAttribute("type") === "checkbox" && !cb.classList.contains("sr-only"));
    // Auto mode is on by default when no config, so first we need to turn it off
    const autoToggle = screen.getAllByRole("checkbox").find((cb) => cb.classList.contains("sr-only"));
    if (autoToggle) fireEvent.click(autoToggle);

    // Now select models
    const gpt4o = checkboxes.find((cb) => cb.closest("label")?.textContent?.includes("GPT-4o"));
    const claude = checkboxes.find((cb) => cb.closest("label")?.textContent?.includes("Claude 3 Opus"));
