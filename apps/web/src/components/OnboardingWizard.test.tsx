...
const POPULAR_PROVIDERS = [
  { id: "openai", name: "OpenAI", npm: "@ai-sdk/openai", doc: "", env: [], modelCount: 12, popular: true, models: [] },
...
const SAMPLE_MODELS = [
  { id: "deepseek-chat", name: "DeepSeek Chat", provider: "deepseek", description: "text", contextWindow: 64000, capabilities: ["text"] },
  { id: "deepseek-reasoner", name: "DeepSeek Reasoner", provider: "deepseek", description: "reasoning", contextWindow: 64000, capabilities: ["text", "reasoning"] },
];
...
describe("OnboardingWizard", () => {
  ...
  it("clicking Saltar (then Siguiente) advances to step 4 without successful validation", async () => {
    ...
    fireEvent.click(screen.getByTestId("wizard-next"));
    await waitFor(() => expect(screen.getByText(/elegí tu modelo por defecto/i)).toBeInTheDocument());
  });
...
describe("OnboardingWizard — connect on step 3 → 4 transition", () => {
  ...
  it("step 3 Siguiente shows 'Conectando…' while handleConnect is in-flight", async () => {
    ...
    mockHandleConnect.mockImplementationOnce(
      () => new Promise<never>(() => {}),
    );
    renderWizard();
    fireEvent.click(screen.getByTestId("provider-card-deepseek"));
    ...
    await waitFor(() =>
      expect(screen.getByTestId("wizard-next")).toHaveTextContent("Conectando…"),
    );
  });
