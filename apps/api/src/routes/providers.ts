...
router.get("/models/:provider/:modelId/variants", authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
...
router.get("/connected", authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const connectedProviderIds = new Set(
      connectedProviders.map((p) => p.providerId)
    );
    ...
    if (options) {
      try {
        ...
        if (err instanceof Error && "issues" in err) {
          ...
          res.status(400).json({ error: "Invalid options: " + (err instanceof Error ? err.message : "unknown error") });
        }
        return;
      }
    }
...
router.post("/test-connection", authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (!actualApiKey) {
      ...
      if (!config) {
        res.status(400).json({ error: "Provider not configured. Provide apiKey or save the provider first." });
        return;
      }
    ...
    await provider.chat(
      { messages: [{ role: "user", content: "Hi" }], model: provider.getCapabilities()[0]?.modelId ?? "" },
      actualApiKey
    );
