...
function SourceFavicon({ host, size = 16 }: { host: string; size?: number }) {
  ...
  return (
    <img
      ...
      style={{ width: size, height: size, objectFit: "cover", backgroundColor: "var(--bg-elevated)" }}
    />
  );
}
            ...
            <span className="flex items-center" style={{ marginLeft: 2 }}>
              {preview.map((s, i) => (
                <span
                  ...
                  style={{ marginLeft: i === 0 ? 0 : -5, zIndex: preview.length - i, boxShadow: "0 0 0 1.5px var(--bg-app)" }}
                >
      ...
      {open && hasSources && (
        ...
        >
          {sources.map((s) => (
            ...
            >
              <SourceFavicon host={s.host} />
              <span className="truncate" style={{ maxWidth: 360, color: "var(--text-2)" }}>{s.title}</span>
              <span className="shrink-0" style={{ color: "var(--text-4)" }}>{s.host}</span>
            </a>
          ))}
        </div>
      )}
...
function collectPythonRuns(toolCalls: ToolCallRecord[]): PythonRun[] {
  return toolCalls.map((tc) => {
    ...
    const res = tc.result as
      | { stdout?: string; result?: string; error?: string; truncated?: boolean; timedOut?: boolean }
      | undefined;
    return {
      code: args?.code ?? "",
      stdout: res?.stdout,
        ...
        >
          {runs.map((run, i) => (
            <div key={i} className="flex flex-col gap-1">
              <span className="text-xs" style={{ color: "var(--text-4)" }}>Código</span>
              <pre style={preStyle}>{run.code}</pre>
              {run.error ? (
                <>
                  <span className="text-xs" style={{ color: "var(--m-rose)" }}>Error</span>
                  <pre style={{ ...preStyle, color: "var(--m-rose)" }}>{run.error}</pre>
                </>
              ) : (
                <>
                  <span className="text-xs" style={{ color: "var(--text-4)" }}>Salida</span>
  ...
  return (
    <div className="mb-3">
      <button
        ...
        aria-expanded={open}
      >
        {active ? <ThinkingDots /> : <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: "var(--accent)" }} />}
      ...
      {open && (
        <div
          ...
          style={{ borderLeft: "2px solid var(--accent-line)", color: "var(--text-3)", lineHeight: 1.6 }}
        >
          {reasoning}
          {active && <span className="animate-pulse" style={{ color: "var(--text-4)" }}>▍</span>}
        </div>
      )}
    </div>
  );
...
function ImageAttachments({ attachments, align }: { attachments: Attachment[]; align: "end" | "start" }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <path d="M9 13h1.5a1.5 1.5 0 0 1 0 3H9zM9 13v5" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
...
function FileAttachments({ attachments, align }: { attachments: Attachment[]; align: "end" | "start" }) {
  if (attachments.length === 0) return null;
  return (
    <div className={`flex flex-wrap gap-2 ${align === "end" ? "justify-end" : "justify-start"}`}>
      {attachments.map((a, i) =>
        a.type === "pdf" ? (
          ...
          <div
            key={i}
            className="flex items-center gap-2 px-3 py-2"
            style={{ backgroundColor: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", maxWidth: 240 }}
          >
            <span style={{ color: "var(--text-3)" }}><FileIcon className="w-4 h-4" /></span>
            <span className="truncate text-xs" style={{ color: "var(--text-2)" }}>{a.name ?? "archivo"}</span>
          </div>
        ),
      )}
    </div>
...
export function ChatMessageItem({ msg, userName, streaming, isLast, isNew, onRegenerate }: ChatMessageItemProps) {
    ...
    <div className={`group py-3 ${isNew ? "animate-fade-in-up" : ""}`}>
      ...
      {showTyping && !(msg.reasoning && !msg.content) && (
        <div className="flex items-center gap-2 mb-2">
          <ThinkingDots />
          <span className="text-xs" style={{ color: "var(--text-4)" }}>{msg.content ? "Respondiendo…" : "Pensando…"}</span>
        </div>
      )}
      ...
      {isCouncil ? (
        ...
        <div
          className="pl-3"
          style={{ borderLeft: "2px solid var(--m-rose)", color: "var(--m-rose)", fontSize: 15, lineHeight: 1.5 }}
        >
          {msg.content}
        </div>
      ) : (
      ...
      {showActions && (
        <div className="mt-2 flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity duration-200">
          ...
          <button
            ...
            onMouseEnter={(e) => { if (feedback !== "helpful") (e.currentTarget as HTMLButtonElement).style.color = "var(--text-2)"; }}
            onMouseLeave={(e) => { if (feedback !== "helpful") (e.currentTarget as HTMLButtonElement).style.color = "var(--text-4)"; }}
          ...
          <button
            ...
            onMouseEnter={(e) => { if (feedback !== "unhelpful") (e.currentTarget as HTMLButtonElement).style.color = "var(--text-2)"; }}
            onMouseLeave={(e) => { if (feedback !== "unhelpful") (e.currentTarget as HTMLButtonElement).style.color = "var(--text-4)"; }}
