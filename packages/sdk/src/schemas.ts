...
function validateHeaders(headers: Record<string, string>): void {
  ...
  for (const [name, value] of entries) {
    ...
    if (!ALLOWED_CUSTOM_HEADERS.has(lower)) {
      throw new Error(`Custom header not in allowlist: ${name}. Allowed: ${[...ALLOWED_CUSTOM_HEADERS].join(", ")}`);
    }
  }
}
...
export const ProviderOptionsSchema = z.object({
  baseURL: z.string()
    ...
    .refine((url) => {
      ...
      if (parsed.protocol !== "https:" && parsed.hostname !== "localhost" && !parsed.hostname.startsWith("127.") && parsed.hostname !== "[::1]") {
        return false;
      }
  ...
  apiEndpoint: z.string()
    .min(1)
    .max(100)
    .regex(/^\/[a-zA-Z0-9/._-]+$/, "apiEndpoint must be a valid API path (e.g. /v1/chat/completions)")
