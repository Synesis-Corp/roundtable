...
export default function SettingsPage() {
  ...
  const renderProviderRow = (provider: typeof providers[number], withCodex: boolean) => (
    <ProviderRow
      ...
      onConnect={() => handleConnect(provider.id, apiKeys[provider.id]?.trim() || "", providerOptions[provider.id])}
      ...
      onToggleAdvanced={() => setAdvancedOpen((prev) => ({ ...prev, [provider.id]: !prev[provider.id] }))}
      codexConnecting={withCodex ? codexConnecting : false}
      onCodexConnect={withCodex ? handleCodexStart : () => {}}
      onManageModels={isConnected(provider.id) ? () => setActiveModelsProvider(provider.id) : undefined}
    />
  );
      ...
      <div className="mx-auto px-4 py-8 max-w-4xl">
        ...
        {codexNotice && activeTab === "providers" && (
          <div
            ...
            style={{
              ...
              border: codexNotice.type === "error" ? "1px solid rgba(208,119,160,0.2)" : "1px solid rgba(92,176,139,0.2)",
              backgroundColor: codexNotice.type === "error" ? "rgba(208,119,160,0.06)" : "rgba(92,176,139,0.06)",
              padding: "12px 16px",
            }}
          >
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm" style={{ color: codexNotice.type === "error" ? "var(--m-rose)" : "var(--m-green)" }}>
                {codexNotice.text}
              </p>
              <button
        ...
        {activeTab === "providers" && (
          <div className="space-y-8" role="tabpanel">
            ...
            {providersError && (
              <div style={{ borderRadius: "var(--r-md)", border: "1px solid rgba(208,119,160,0.2)", backgroundColor: "rgba(208,119,160,0.06)", padding: "12px 16px" }}>
                <p className="text-sm" style={{ color: "var(--m-rose)" }}>{providersError}</p>
              </div>
            )}

            {(providersLoading || userProvidersLoading) && filteredPopular.length === 0 && filteredOther.length === 0 && (
              <div className="space-y-3">
                {Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className="animate-shimmer" style={{ height: 120, borderRadius: "var(--r-md)", backgroundColor: "var(--bg-surface)" }} />
                ))}
              </div>
            )}

            {connectedCount === 0 && !providersLoading && !userProvidersLoading && (
              <div className="flex flex-col items-center justify-center py-12" style={{ borderRadius: "var(--r-md)", border: "1px dashed var(--border)", backgroundColor: "var(--bg-surface)" }}>
                <div className="flex items-center justify-center mb-4" style={{ width: 48, height: 48, borderRadius: "var(--r-md)", backgroundColor: "var(--accent-quiet)" }}>
            ...
            {otherUnconnected.length > 0 && (
              isBrowsing || showAllProviders ? (
                ...
                <button
                  ...
                  style={{ border: "1px dashed var(--border)", borderRadius: "var(--r-md)", color: "var(--text-2)", backgroundColor: "var(--bg-surface)" }}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <span
                  ...
                  style={{
                    backgroundColor: councilConfig?.mode === "manual" ? "var(--accent-quiet)" : "var(--bg-elevated)",
                    color: councilConfig?.mode === "manual" ? "var(--accent-text)" : "var(--text-2)",
