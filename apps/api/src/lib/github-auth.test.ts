...
describe("buildAuthorizeUrl", () => {
  it("constructs a valid GitHub authorize URL with all required params", () => {
    const url = buildAuthorizeUrl("state-abc", "https://app.test/api/auth/github/callback", "client-123");
    const parsed = new URL(url);
    expect(parsed.origin + parsed.pathname).toBe("https://github.com/login/oauth/authorize");
    expect(parsed.searchParams.get("client_id")).toBe("client-123");
    expect(parsed.searchParams.get("redirect_uri")).toBe("https://app.test/api/auth/github/callback");
...
describe("exchangeCodeForToken", () => {
  ...
  it("throws when GitHub returns a 200 with an error field (OAuth2 error envelope)", async () => {
    ...
    await expect(
      exchangeCodeForToken("expired", "https://x/cb", "id", "sec", fetchMock as unknown as typeof fetch),
...
describe("fetchGitHubUser", () => {
  ...
  it("GETs /user with the bearer token and returns the parsed body", async () => {
    const user = { id: 12345, login: "octocat", name: "The Octocat", email: "[email protected]", avatar_url: "https://gh/ava" };
  ...
  it("throws on non-2xx", async () => {
    ...
    await expect(
      fetchGitHubUser("bad", fetchMock as unknown as typeof fetch),
...
describe("fetchPrimaryEmail", () => {
  ...
  it("returns null when no email is primary+verified", async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(
      jsonResponse([{ email: "[email protected]", primary: true, verified: false, visibility: null }]),
    );
    const out = await fetchPrimaryEmail("tok", fetchMock as unknown as typeof fetch);
    expect(out).toBeNull();
  });
  ...
  it("throws on non-2xx", async () => {
    ...
    await expect(
      fetchPrimaryEmail("tok", fetchMock as unknown as typeof fetch),
