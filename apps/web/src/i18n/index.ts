...
void i18n
  ...
  .init({
    ...
    // Resources are bundled inline (no async backend), so init runs
    // synchronously and `t` works immediately — important for tests and to
    // avoid a flash of translation keys on first paint.
    initImmediate: false,
    interpolation: {
