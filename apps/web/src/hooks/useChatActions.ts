...
export function useChatActions({
  ...
  const handleSend = useCallback((text: string) => {
    ...
    if (!token) {
      setMessages((prev) => [...prev, { id: `err-${Date.now()}`, role: "assistant", content: "Please log in to send messages.", isError: true }]);
      return;
    }
    ...
    setFiles([]);
    setInputText("");
  }, [messages, selectedModel, multiMode, incognito, userProviders, effortSpec, selectedEffort, startStream, conversationId, files, setMessages, setError, setMultiInfo, setCouncilInfo, setFiles, setInputText]);
  ...
  }, [messages, selectedModel, multiMode, incognito, effortSpec, selectedEffort, startStream, conversationId, setMessages, setError, setMultiInfo, setCouncilInfo]);
  ...
  const handleStopStream = useCallback(() => {
    ...
    setMessages((prev) => {
      ...
      if (last && last.role === "assistant" && last.content === "" && last.id.startsWith("pending-")) {
