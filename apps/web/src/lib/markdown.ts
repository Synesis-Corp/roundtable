...
export function renderMarkdown(text: string): string {
  ...
  html = html.replace(
    /`([^`]+)`/g,
    '<code class="bg-gray-800 text-green-300 px-1.5 py-0.5 rounded text-sm">$1</code>'
  );
  ...
  html = html.replace(
    /^&gt; (.+)$/gm,
    '<blockquote class="border-l-4 border-gray-600 pl-4 my-2 text-gray-400">$1</blockquote>'
  );
